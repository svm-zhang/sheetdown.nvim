...
function M.parse_range(expression)
	...
	if start_index < 1 or finish_index < start_index then
		return nil,
			"Invalid range. The start must be at least 1 and the end must be greater than or equal to the start."
	end
...
function M.read_headers(path, delimiter)
	...
	if not headers then
		return nil,
			("Malformed header at line %d: %s"):format(
				line_number,
				parse_error
			)
	end
		...
		if not is_blank(raw_line) then
			if not header_seen then
				...
				if not fields then
					handle:close()
					return nil,
						("Failed to parse %s at line %d in %s. %s"):format(
							csv_name(delimiter),
							line_number,
							path,
							parse_error
				...
				if #fields ~= expected_columns then
					handle:close()
					return nil,
						("Failed to parse %s at line %d in %s. Expected %d columns but found %d."):format(
							csv_name(delimiter),
							line_number,
							path,
							expected_columns,
				...
				elseif row_selection.mode == "range" then
					if
						data_row_index >= row_selection.start
						and data_row_index <= row_selection.finish
					then
						rows[#rows + 1] = fields
					end
					...
					handle:close()
					return nil,
						("Unsupported row mode: %s"):format(
							tostring(row_selection.mode)
						)
				end
			end
		end
